...
# #                    lat_destination = 1,
# #                    long_destination = 1)
# #   #ldf <- as.locationdataframe(df)
# #   
# #   #mm <- as.movementmatrix(data.frame(origin = c('a', 'b'),
# #                                      destination = c('b', 'a'),
# #                                      movement = c(1, 1)))
# #   #and  <- extractArgumentsFromFormula(mm ~ ldf)
#   locations <- c("a","b","c")
...
#   class(matrix) <- c('movementmatrix', 'matrix')
#   
#   print(data)
#   print(matrix)
#   ans  <- extractArgumentsFromFormula(matrix ~ data)
#   #expect_error(movement(mm ~ ldf, "original radiation"), "Error: Unknown flux model type given. The input 'flux_model' has to be a flux object.")
# })
...
# 		},
# 		analysepredictionusingdpois = function(x, y) return (1),
# 		actual_movement_object <- movement(locationdataframe, movement_matrix, uniform.selection()),
# 		expect_equal(actual_movement_object$optimisationresults$par, c(theta=0.9))
# 	)
# })
# 
...
# 		},
# 		analysepredictionusingdpois = function(x, y) return (1),
# 		actual_movement_object <- movement(locationdataframe, movement_matrix, radiation.with.selection()),
...
# 		},
# 		analysepredictionusingdpois = function(x, y) return (1),
# 		actual_movement_object <- movement(locationdataframe, movement_matrix, intervening.opportunities()),
...
# 		},
# 		analysepredictionusingdpois = function(x, y) return (1),
# 		actual_movement_object <- movement(locationdataframe, movement_matrix, gravity()),
...
#   },
#   analysepredictionusingdpois = function(x, y) return (1),
#   actual_movement_object <- movement(locationdataframe, movement_matrix, gravity.with.distance()),
